	...
	public String retornaScriptInsertUpdate(String chave) {
		...
		if (chave.equals("FILIAL")) {
			...
			return " insert into g4.tb_dim_filialg4 (id, empresa, codigo, cidadecodibge, razaosocial) "
					+ " values ('[ID]','[EMPRESA]','[CODIGO]',[CIDADEIBGE],'[RAZAOSOCIAL]') " 
			        + " on conflict (id) "
					+ " do " 
					+ " update " 
					+ "    set "
			...
			return " insert into g4.tb_dim_localizacaog4(id,uf,nome,capital,longitude,latitude,microregiao,mesoregiao,regiao,estado,populacao,pais) "
					+ " values('[ID]','[UF]','[NOME]','[CAPITAL]','[LONGITUDE]','[LATITUDE]','[MICROREGIAO]','[MESOREGIAO]','[REGIAO]','[ESTADO]','[POPULACAO]','[PAIS]') "
					+ "  on conflict (id) " 
					+ "  do " 
					+ " update " 
					+ "     set id          = '[ID]'            , "
			...
			return "insert into g4.tb_fato_entradag4 (id,empresa,filial_id,dtemissao,nronota,parcelamento,produto_id,quantidade,precocompra,vlcredicms,tipo,dtcancel,dtultimaalteracao)"
